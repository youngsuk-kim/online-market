...
class ProductMongoRepositoryAdapterTest {
    ...
    // 테스트 대상
    private val productMongoRepositoryAdapter = ProductMongoRepositoryAdapter(productMongoRepository)
    ...
    fun ID로_상품을_조회한다() {
        // Given
        every { productMongoRepository.findById(productId.toString()) } returns Optional.of(productDocument)

        // When
        val result = productMongoRepositoryAdapter.findById(productId)

        // Then
        assertNotNull(result)
        assertEquals(productId, result?.id)
        assertEquals("테스트 상품", result?.name)
        verify { productMongoRepository.findById(productId.toString()) }
    }

    @Test
    fun 상품이_없으면_null을_반환한다() {
        // Given
        every { productMongoRepository.findById(productId.toString()) } returns Optional.empty()
    ...
    fun 상품을_저장한다() {
        ...
        // When
        productMongoRepositoryAdapter.save(product)
